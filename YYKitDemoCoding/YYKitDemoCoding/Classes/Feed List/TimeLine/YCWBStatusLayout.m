...
@implementation YCWBStatusLayout
...
- (void)_layoutName {
    ...
    if (user.userVerifyType == WBUserVerifyTypeOrganization) {
        UIImage *blueVImage = [YCWBStatusHelper imageNamed:@"avatar_enterprise_vip"];
    ...
    // VIP
    if (user.mbrank > 0) {
        UIImage *yelllowVImage = [YCWBStatusHelper imageNamed:[NSString stringWithFormat:@"common_icon_membership_level%d",user.mbrank]];
        if (!yelllowVImage) {
            yelllowVImage = [YCWBStatusHelper imageNamed:@"common_icon_membership"];
        }
...
- (void)_layoutSource {
    ...
    NSString *createTime = [YCWBStatusHelper stringWithTimelineDate:_status.createdAt];
...
- (void)_layoutToolbar {
    ...
    NSMutableAttributedString *repostText = [[NSMutableAttributedString alloc] initWithString:_status.repostsCount <= 0 ? @"转发" : [YCWBStatusHelper shortedNumberDesc:_status.repostsCount]];
    ...
    NSMutableAttributedString *commentText = [[NSMutableAttributedString alloc] initWithString:_status.commentsCount <= 0 ? @"评论" : [YCWBStatusHelper shortedNumberDesc:_status.commentsCount]];
    ...
    NSMutableAttributedString *likeText = [[NSMutableAttributedString alloc] initWithString:_status.attitudesCount <= 0 ? @"赞" : [YCWBStatusHelper shortedNumberDesc:_status.attitudesCount]];
    likeText.font = font;
...
- (NSMutableAttributedString *)_textWithStatus:(WBStatus *)status
        ...
        do {
            ...
            if ([text attribute:YYTextHighlightAttributeName atIndex:range.location] == nil) {
                ...
                if (wburl.urlTypePic.length) {
                    // 链接头部有个图片附件 (要从网络获取)
                    NSURL *picURL = [YCWBStatusHelper defaultURLForImageURL:wburl.urlTypePic];
    ...
    // 匹配 @用户名
    NSArray *atResults = [[YCWBStatusHelper regexAt] matchesInString:text.string options:kNilOptions range:text.rangeOfAll];
    ...
    // 匹配 [表情]
    NSArray *emoticonResults = [[YCWBStatusHelper regexEmoticon] matchesInString:text.string options:kNilOptions range:text.rangeOfAll];
    ...
    for (NSTextCheckingResult *emo in emoticonResults) {
        ...
        NSString *imagePath = [YCWBStatusHelper emoticonDic][emoString];
        UIImage *image = [YCWBStatusHelper imageWithPath:imagePath];
        if (!image) continue;
...
- (NSAttributedString *)_attachmentWithFontSize:(CGFloat)fontSize imageURL:(NSString *)imageURL shrink:(BOOL)shrink {
    ...
    attachment.imageURL = [YCWBStatusHelper defaultURLForImageURL:imageURL];
